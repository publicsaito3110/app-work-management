$(function() {
	...
	$("#schedule-report-seach-count-btn").on("click", function (){
		...
		// 非同期処理後、テーブルに結果を追加
		$("#tbody-count").children("tr:last").load("/schedule-report/search-count?ym=" + ym + " .search-td1, .search-td2");
	});
	...
	$("#schedule-report-seach-time-btn").on("click", function (){
		...
		// 非同期処理後、テーブルに結果を追加
		$("#tbody-time").children("tr:last").load("/schedule-report/search-time?ym=" + ym + " .search-td1, .search-td2");
	});
});